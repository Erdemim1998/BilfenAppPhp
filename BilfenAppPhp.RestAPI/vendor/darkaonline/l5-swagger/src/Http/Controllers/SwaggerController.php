...
class SwaggerController extends BaseController
{
    ...
    public function api(Request $request)
    {
        ...
        if ($proxy = $config["defaults"]['proxy']) {
            if (! is_array($proxy)) {
                $proxy = [$proxy];
            }
        ...
        return ResponseFacade::make(
            view('l5-swagger::index', [
                ...
                'urlToDocs' => $urlToDocs,
                'operationsSorter' => $config["defaults"]['operations_sort'],
                'configUrl' => $config["defaults"]['additional_config_url'],
                'validatorUrl' => $config["defaults"]['validator_url'],
